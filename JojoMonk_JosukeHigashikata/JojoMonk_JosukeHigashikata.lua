...
local last_heal_signal = 0;
local last_unit_combat_signal = 0;
...
--f:RegisterEvent('UNIT_DAMAGED', 'player')
...
--COMBAT_LOG_EVENT_UNFILTERED no longer exists since the patch 12.0.0
...
-- Instead of COMBAT_LOG_EVENT_UNFILTERED, we use UNIT_COMBAT and UNIT_HEALTH to trigger hurt sounds. It is less precise, but it works for now, until Blizzard possibly adds a new event for combat log in the future.
function f:OnCombatEvent(event, unit)
	
	if(JojoMonk_Config.ActiveDeadEliteSFX == true) then
		
		targetGUID = UnitGUID("target")
		if event == "UNIT_DIED" and lastTargetInfos[targetGUID] then
			
			if(lastTargetInfos[targetGUID][1] == "player") then
				repeat
			...
			elseif(lastTargetInfos[targetGUID][1] ~= "normal" and lastTargetInfos[targetGUID][1] ~= "minus" and lastTargetInfos[targetGUID][1] ~= "trivial" ) then
				repeat
	...
	if(JojoMonk_Config.ActiveHurtSFX == true) then
		...
		if event == 'UNIT_HEALTH' and unit == 'player' then
			...
			if GetTime() - last_heal_signal < 2 then
				return
			end
